...
const run = async () => {
  try {
    ...
    const reportPath = path.join(reportsDir, "review-workflow-report.json");
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");

    console.log("\n------------------------------------------------");
    console.log("ADMIN REVIEW WORKFLOW CALCULATION COMPLETE");
    console.log("------------------------------------------------");
    console.log(`Total: ${summary.totalColleges}`);
    console.log(`Pending: ${summary.pendingReview} | Not Required: ${summary.notRequired}`);
    console.log("Queue Metrics:", JSON.stringify(summary.reviewQueueMetrics, null, 2));
    console.log(`Report: ${reportPath}`);
    console.log("------------------------------------------------\n");

    process.exit(0);
  } catch (error) {
    console.error("Error during admin review workflow calculation:", error);
    process.exit(1);
  }
};
